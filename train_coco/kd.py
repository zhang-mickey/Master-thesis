...
def parse_args():
    ...
    parser.add_argument("--use_crop", type=bool, default=False, help="use cropped image or not")
    ...
    parser.add_argument("--coco14", type=bool, default=True, help="use COCO14 dataset or not")
    parser.add_argument("--coco14_root", type=str, default=os.path.join(project_root, "train_coco/"),
                        help="path to VOC2012 dataset root directory")
    parser.add_argument("--coco14_list", type=str, default=os.path.join(project_root, "train_coco/"),
                        help="path to VOC2012 dataset list directory")
    parser.add_argument("--coco14_split", type=str, default="train",
...
if __name__ == "__main__":
    ...
    if args.coco14:
        train_dataset = COCO14SegDataset(
            root_dir=args.coco14_root,
            name_list_dir=args.coco14_list,
            split=args.coco14_split,
            stage='train',
            crop_size=args.crop_size,
            aug=True  # Enable augmentation for training
        )
        # For testing/validation
        test_dataset = COCO14SegDataset(
            root_dir=args.coco14_root,
            name_list_dir=args.coco14_list,
    ...
    model_vit = choose_backbone(args.backbone, num_classes=args.num_class)
    model_resnet50 = choose_backbone("resnet50", num_classes=args.num_class)
    model_vit = model_vit.to(device)

    save_resnet_path = os.path.join(
        os.path.dirname(args.save_model_path),
        f"resnet50_4_model_pcm.pth"
    )
    ...
    if args.backbone == 'vit_b' or args.backbone == 'vit_s':
        ...
        for epoch in range(1, (args.num_epochs + 1)):
            ...
            avg_loss, kd_loss, cls_vit_loss, avg_acc, avg_f1 = avg_meter.get('loss', 'kd_loss', 'cls_loss', 'accuracy',
                                                                             'f1')
            print(
                f"Epoch [{epoch}/{args.num_epochs}], Loss: {avg_loss:.4f},kd Loss: {kd_loss:.4f},cls Loss: {cls_vit_loss:.4f}, Avg Accuracy: {avg_acc:.2f}%,Avg f1:{avg_f1:.4f}")
