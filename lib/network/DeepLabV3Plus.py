...
class DeepLabV3Plus(nn.Module):
    def __init__(self, num_classes=1, pretrained=True, backbone_name='deeplabv3plus_resnet101'):
        ...
        if backbone_name == 'deeplabv3plus_resnet50':
            ...
            resnet = models.resnet101(weights=ResNet101_Weights.IMAGENET1K_V1 if pretrained else None)
    ...
    def forward(self, x, return_features=False):
        ...
        print("deeplabv3+ output shape", output.shape)
        if return_features:
            features = {
                'low_level': x1,
                'high_level': x4,
...
class DecoderModule(nn.Module):
    def __init__(self, in_channels_high, in_channels_low, num_classes):
        ...
        # The high-level features coming from the ASPP module
        self.refine_high = nn.Sequential(
            nn.Conv2d(in_channels_high, 256, kernel_size=1, bias=False),
            nn.BatchNorm2d(256),
            nn.ReLU()
        )
        ...
        self.fusion = nn.Sequential(
            nn.Conv2d(256 + in_channels_low, 256, kernel_size=3, padding=1, bias=False),
    ...
    def forward(self, high_level_features, low_level_features):
        high_level_features = self.refine_high(high_level_features)
