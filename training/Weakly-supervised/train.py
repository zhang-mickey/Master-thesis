...
def parse_args():
    parser = argparse.ArgumentParser(description="Weakly Supervised learning")
    # Dataset
    ...
    # parser.add_argument("--image_folder", type=str,
    #     default=os.path.join(project_root, "smoke-segmentation.v5i.coco-segmentation/test/"), help="Path to the image dataset folder")
    ...
    parser.add_argument("--save_model_path", type=str, default=os.path.join(project_root, "model/weakly_model.pth"),
    ...
    parser.add_argument('--max_sample', default=256, type=int,
                        help='contrastive learning sample.')
    ...
    parser.add_argument('--loss_lambda', default=0.5, type=float,
                        help='scale factor for contrastive loss:0 wihtout contrastive loss')
...
if __name__ == "__main__":
    ...
    # transform=image_transform, mask_transform=mask_transform,image_ids=val_ids)
    ...
    # Keep using regular dataset for validation and testing
    ...
    # transform=image_transform, mask_transform=mask_transform, image_ids=test_ids)
    # # Create DataLoaders
    # train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True)
    ...
    for epoch in range(1, (args.num_epochs + 1)):
        ...
        for i, (images, labels, _, masks) in enumerate(train_loader):
            ...
            low_level_features = feature_maps['processed_low_level']
            ...
            # contrastive_loss = pixel_contrastive_loss(decoder_features, masks,temperature=0.1,max_samples=args.max_sample)
            ...
            if args.loss_lambda == 0.0:
                ...
                contrastive_loss = pixel_contrastive_loss(decoder_features, masks, temperature=0.1,
                                                          max_samples=args.max_sample)
                print("segmentation_loss", segmentation_loss)
                print("contrastive_loss", contrastive_loss)
                loss = segmentation_loss + (args.loss_lambda) * contrastive_loss
            loss.backward()
            # update weights
            optimizer.step()
        ...
        with torch.no_grad():
            for images, labels, _, masks in val_loader:
