...
def parse_args():
    ...
    parser.add_argument("--save_model_path", type=str,
                        default=os.path.join(project_root, "model/model_classification_mix.pth"),
...
def generate_cam(model, feature_map, class_weights):
    """
    输入:
        model: ViT 模型（用于获取分类头权重）
        feature_map: patch token 特征图（来自 forward_features）
        class_weights: 分类头权重（如 model.head.weight.data）
    输出:
        cam: Class Activation Map (B, num_classes, H, W)
    """
    B, embed_dim, H, W = feature_map.shape
    num_classes = class_weights.shape[0]

    # 展平特征图
    patch_tokens = feature_map.view(B, embed_dim, -1)  # (B, embed_dim, H*W)

    # CAM = class_weights @ patch_tokens
    cam = torch.matmul(class_weights, patch_tokens)  # (B, num_classes, H*W)
    cam = cam.view(B, num_classes, H, W)  # (B, num_classes, H, W)

    # 归一化 CAM（ReLU + L2 归一化）
    cam = F.relu(cam, inplace=True)
    cam = cam / (cam.norm(p=2, dim=(2, 3), keepdim=True) + 1e-5)

    # 上采样到原始图像尺寸（如 1920x1080）
...
if __name__ == "__main__":
    ...
    if args.use_crop:
        strong_dataset = mix_CropDataset(
        ...
        weak_pos_dataset = mix_CropDataset(
            image_dir=args.Dutch_positive_path,
            weak=True,
            label=1,
            transform=image_transform,
            mask_transform=mask_transform,
        ...
        weak_neg_dataset = mix_CropDataset(
            image_dir=args.Dutch_negative_path,
            weak=True,
            label=0,
            transform=image_transform,
            mask_transform=mask_transform,
        ...
        weak_dataset = ConcatDataset([weak_pos_dataset, weak_neg_dataset])
        weak_size = len(weak_dataset)

        strong_sample_size = weak_size // 5
        strong_indices = random.sample(range(len(strong_dataset)), strong_sample_size)
        strong_subset = Subset(strong_dataset, strong_indices)
        train_dataset = ConcatDataset([weak_dataset, strong_subset])
        ...
        print("weak_size:", weak_size)
        print("strong_sample_size:", strong_sample_size)
    ...
    avg_meter = AverageMeter('loss', 'cls_loss', 'seg_loss', 'f1', 'accuracy', 'loss_entropy', 'bg_loss',
                             'consistency_loss')

    if args.backbone == 'resnet101' or args.backbone == 'resnet50_raw' or args.backbone == 'resnet50':
        ...
        for epoch in range(1, (args.num_epochs + 1)):
            ...
            for batch_idx, (images, labels, _, mask) in enumerate(train_loader):
                ...
                outputs_aug = torch.sigmoid(outputs_aug)
            ...
            avg_loss, consistency_loss, cls_loss, avg_acc, avg_f1 = avg_meter.get('loss', 'consistency_loss',
                                                                                  'cls_loss', 'accuracy', 'f1')
            print(
                f"Epoch [{epoch}/{args.num_epochs}], Loss: {avg_loss:.4f},consistency_loss Loss: {consistency_loss:.4f},cls Loss: {cls_loss:.4f}, Avg Accuracy: {avg_acc:.2f}%,Avg f1:{avg_f1:.4f}")
    ...
    elif args.backbone == 'vit_s' or args.backbone == 'vit_b':
        ...
        for epoch in range(1, (args.num_epochs + 1)):
            ...
            for batch_idx, (images, labels, _, mask) in enumerate(train_loader):
                ...
                cls_loss = criterion(outputs, labels)
                loss = cls_loss
                seg_loss = torch.tensor(0.0, device=outputs.device)
                # print("outputs.shape:", outputs.shape)
                # print("mask.shape:", mask.shape)
                print("maks:", mask)
                if mask is not None and mask.numel() > 0 and mask.max() > 0:
                    class_weights = model.head.weight.data
                    cam = generate_cam(model, feature_maps.detach(), class_weights)
                    seg_loss = dice_loss(cam, mask, num_masks=outputs.size(0))
                    loss += seg_loss
                    model.zero_grad()
                    print("!")
                # consistency_loss = F.mse_loss(outputs, outputs_aug)
            ...
            avg_loss, cls_loss, seg_loss, avg_acc, avg_f1 = avg_meter.get('loss', 'cls_loss', 'seg_loss', 'accuracy',
                                                                          'f1')
            print(
                f"Epoch [{epoch}/{args.num_epochs}], Loss: {avg_loss:.4f},cls Loss: {cls_loss:.4f},seg_loss:{seg_loss:.4f}, Avg Accuracy: {avg_acc:.2f}%,Avg f1:{avg_f1:.4f}")
